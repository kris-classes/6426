...
node_1.mid = node_99
node_99.left = node_300

# Defining the function 'Search the Start Node and the Nodes attached to it', print the Nodes
def DFS(start_node, search_node):
    # current_node = start_node
    # q = Queue.TommyQueue()
    q = stack.Stack()
    q.push(start_node)
    print(f'Added {start_node} to start of stack')
    while q.stack:
        current_node = q.pop()
        print(f'Got {current_node} from the stack')
        if search_node == current_node:
            ...
            if current_node.left is not None:
                ...
                print(f'Added {current_node.left} to the stack')
            if current_node.mid is not None:
                q.push(current_node.mid)
                print(f'Added {current_node.mid} to the stack')
            if current_node.right is not None:
                q.push(current_node.right)
                print(f'Added {current_node.right} to the stack')
        print(f'Current stack {q}')
    print(f'Node not found')

DFS(node_10, Node(300))